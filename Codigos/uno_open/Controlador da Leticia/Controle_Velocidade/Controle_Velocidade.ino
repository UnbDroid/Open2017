...
#include <math.h>
//#include<PinChangeInt.h> // biblioteca para usar mais interrupcoes no uno
//#include <EasyTransfer.h> //Comunicao entre arduinos
...
double graus_esquerda = 0, graus_esquerda_anterior = 0;
double graus_direita = 0, graus_direita_anterior = 0;
...
void setup() {
  ...
  //startComunicacao(); // Inicia comunicacao com outro arduino

}

void loop() {

    // Escreve em um arquivo de texto no cartao SD (feedback de teste) -----
    /*Nessa parte ele declara o SD e inicializa o arquivo */
   ...
   if (millis() - tempo > 5) 
   {
      ...
      graus_esquerda = encoder0Pos / (4192.0); // 32*131 - mudamos para 32 pq pega a descida tb
      graus_direita = encoder1Pos / (4192.0); // tem que ter o .0 para o arduino não interpretar como inteiro e ferrar com a precisão
    
      velocidade_esquerda = 1000 * (graus_esquerda - graus_esquerda_anterior) / (tempo_aux);
      velocidade_direita = 1000 * (graus_direita - graus_direita_anterior) / (tempo_aux);
    
      graus_esquerda_anterior = graus_esquerda;
      graus_direita_anterior = graus_direita;
      ...
      Serial.print("Voltas esquerda: ");      Serial.println(graus_esquerda);
      Serial.print("Voltas direita: ");       Serial.println(graus_direita);
      Serial.println(" ");
          
      //envia p outro arduino
  ...
  // Escreve no arquivo txt do cartao SD os feedbacks -------------------------------
  /*if (dataFile) {
